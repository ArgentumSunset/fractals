 ...
 *  @author Kevin Wayne
 */
public final class UltraDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static UltraDraw std = new UltraDraw();
    ...
    ***************************************************************************/
    ...
    private static Image getImage(String filename) {
        ...
        if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
            URL url = UltraDraw.class.getResource(filename);
            if (url != null)
                icon = new ImageIcon(url);
        }
    ...
    ***************************************************************************/
    ...
    public void actionPerformed(ActionEvent e) {
        FileDialog chooser = new FileDialog(UltraDraw.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        String filename = chooser.getFile();
        if (filename != null) {
            UltraDraw.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    ***************************************************************************/
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = UltraDraw.userX(e.getX());
            mouseY = UltraDraw.userY(e.getY());
            mousePressed = true;
        }
    }
    ...
    *  as an argument. It can handle action keys
     ...
     */
    public static void main(String[] args) {
        UltraDraw.square(.2, .8, .1);
        UltraDraw.filledSquare(.8, .8, .2);
        UltraDraw.circle(.8, .2, .2);

        UltraDraw.setPenColor(UltraDraw.BOOK_RED);
        UltraDraw.setPenRadius(.02);
        UltraDraw.arc(.8, .2, .1, 200, 45);

        // draw a blue diamond
        UltraDraw.setPenRadius();
        UltraDraw.setPenColor(UltraDraw.BOOK_BLUE);
        double[] x = { .1, .2, .3, .2 };
        double[] y = { .2, .3, .2, .1 };
        UltraDraw.filledPolygon(x, y);

        // text
        UltraDraw.setPenColor(UltraDraw.BLACK);
        UltraDraw.text(0.2, 0.5, "black text");
        UltraDraw.setPenColor(UltraDraw.WHITE);
        UltraDraw.text(0.8, 0.8, "white text");
    }
